...
use wgpu::{
    ...
    DepthStencilState, Extent3d, Face, FilterMode, FragmentState, FrontFace, MultisampleState,
    PipelineLayoutDescriptor, PrimitiveState, Queue, RenderPipeline, RenderPipelineDescriptor,
    SamplerDescriptor, ShaderModule, ShaderStages, StencilState, TextureDimension, TextureFormat,
...
impl Shader {
    ...
    pub fn write_transform_data(&self, queue: &Queue, offset: BufferAddress, matrix: &[f32; 16]) {
        queue.write_buffer(&self.w_buffer, offset, bytemuck::bytes_of(matrix));
    }
    pub fn write_rotation_data(&self, queue: &Queue, offset: BufferAddress, matrix: &[f32; 16]) {
        queue.write_buffer(&self.r_buffer, offset, bytemuck::bytes_of(matrix));
    }
    pub fn write_time_data(&self, queue: &Queue, time: f32) {
        queue.write_buffer(
    ...
    pub fn write_camera_data(&self, queue: &Queue, matrix: &[f32; 16]) {
        queue.write_buffer(&self.vp_buffer, 0, bytemuck::bytes_of(matrix));
    }
    pub fn write_light_data(&self, queue: &Queue, lights: &Vec<Light>) {
        queue.write_buffer(
